...
from ruv_dl.search import get_all_programs_by_pattern, program_results
...
def search(patterns: Tuple[str, ...], config: Config) -> str:
...
def download_program(program_ids: Tuple[str, ...], config: Config) -> str:
    ...
    downloaded_episodes = []
    skipped_episodes = []
    iter = tqdm(episodes_to_download)
    try:
        for episode in iter:
            episode = cast(EpisodeDownload, episode)
            iter.set_description(f"Downloading {episode.program_title} - {episode.title}")
    ...
    return f"Downloaded {len(downloaded_episodes)} episodes"
...
def details(program_ids: Tuple[str, ...], config: Config) -> str:
    ...
    return tabulate(tabular_data=rows, headers=p_headers, tablefmt="github")
