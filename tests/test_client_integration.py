...
from ruv_dl.ruv_client import PersistedQueryNotFoundError
...
@pytest.mark.slow
@pytest.mark.asyncio
async def test_persisted_query_error_handling():
    """Test that PersistedQueryNotFoundError provides helpful information"""
    # This test may pass if all queries work, but if any fail, 
    # it should provide detailed error information
    try:
        async with ruv_client.RUVClient() as client:
            programs = await client.get_all_programs()
        
            # Try to get detailed episodes for first program with episodes
            for program in programs.values():
                if program["episodes"]:
                    program_id = program["programID"]
                    # This might raise PersistedQueryNotFoundError
                    await client.get_programs_with_episodes([program_id])
                    break
                
    except PersistedQueryNotFoundError as e:
        # If we get this error, verify it has helpful information
        assert e.operation_name is not None, "Should specify operation name"
        assert e.url is not None, "Should specify URL"
        assert "sha256Hash" in e.url, "URL should contain hash for debugging"
        assert "PersistedQueryNotFound for operation" in str(e)
    
        # Re-raise so the test fails and shows the error
        pytest.fail(f"API query hash needs updating: {e}")
    
    except Exception as e:
        # Handle network errors gracefully
        if "502 Bad Gateway" in str(e) or "HTTPStatusError" in str(type(e).__name__):
            pytest.skip(f"Network error from RÚV API: {e}")
        else:
